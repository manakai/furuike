...
If the final percent-decoded path segment is not the empty string and
identifies a file (e.g. C</path/to/file>), the content of the file is
returned.
...
the default encoding label, is used as the C<charset> parameter value:
  ...
  application/xml-dtd text/x-component text/perl
  application/x-visual-basic-form application/x-visual-basic-form-resource
  application/x-visual-basic-project application/x-visual-basic-window
...
Only C<Header> and C<AddCharset> directives and C<IfModule> lines can
be specified between the lines.  The expression can be one of
followings:
...
Specify that the path should return a redirect to the URL.

The first argument is the status code.  It must be either C<301> or
C<permanent>, C<302> or C<temp>, C<303> or C<seeother>, C<307>, or
C<308>.  The first argument can be omitted if its value is C<302>.

The second argument must be a sequence of one or more non-empty
percent-decoded path segments prefixed by C</>.
...
Same as C<Redirect STATUS /PATH/SEGMENTS URL>.

=item RedirectMatch STATUS /PATH/SEGMENTS/.* URL

Same as C<Redirect STATUS /PATH/SEGMENTS URL> but the last path
segment is C<.*>.  If a redirect is defined using this syntax, any
path whose prefix is equal to the specified path segments are also
redirected.
