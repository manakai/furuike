...
Same as C<Redirect STATUS /PATH/SEGMENTS URL> but any number of path
segments can be followed.  If a redirect is defined using this syntax,
any path whose prefix is equal to the specified path segments are also
redirected.
...
Same as C<Redirect STATUS /PATH/SEGMENTS> but any number of path
segments can be followed.  If a status is defined using this syntax,
any path whose prefix is equal to the specified path segments are also
